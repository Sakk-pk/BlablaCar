// import '../../data/dummy_data.dart';
// import '../model/ride_pref/ride_pref.dart';

// ////
// ///   This service handles:
// ///   - History of the last ride preferences        (to allow users to re-use their last preferences)
// ///   - Curent selected ride preferences.
// ///
// class RidePrefsService {
//   static RidePref? selectedRidePref; // The current selected ride preference

//   static List<RidePref> ridePrefsHistory = fakeRidePrefs;
// }

// abstract class RidePrefListener {
//   void onPrefSelected(RidePref selectedPref);
// }

// class RidePrefStation {
//   late RidePref selectedRidePref;
//   final List<RidePrefListener> _listener = [];

//   void addListener(RidePrefListener listener) {
//     _listener.add(listener);
//   }

//   void setRidePref(RidePref selectRidePref){
//     if (selectRidePref =! selectedRidePref) {
      
//     }
//   }
// }

import 'package:week_3_blabla_project/model/ride/locations.dart';

import '../data/dummy_data.dart';
import '../model/ride_pref/ride_pref.dart';

////
///   This service handles:
///   - History of the last ride preferences        (to allow users to re-use their last preferences)
///   - Curent selected ride preferences.
///
class RidePrefsService {
  static RidePref? selectedRidePref = RidePref(
      departureDate: DateTime.now(),
      requestedSeats: 1,
      departure: Location(name: "Paris", country: Country.uk),
      arrival: Location(
          name: "PhnomPenh",
          country: Country.uk)); // The current selected ride preference

  static List<RidePref> ridePrefsHistory = fakeRidePrefs;
}

